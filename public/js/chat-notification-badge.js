...
(function () {
    ...
    class ChatNotificationBadgeManager {
        ...
        setupGlobalBadgeIntegration() {
            // Ascolta gli eventi del sistema globale
            document.addEventListener('globalBadgeUpdated', (e) => {
                if (e.detail.type === 'chat_message') {
                    const roomId = e.detail.data?.chat_room_id;
                    if (roomId) {
                        this.updateIndividualChatBadge(roomId, e.detail.data?.unread_count || 1);
                    }
                }
            });
