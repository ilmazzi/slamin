...
/* Stili aggiuntivi per lo slider delle foto */
.photos-slider {
    position: relative;
    margin: 0 -10px;
}

.photos-slider .autoplay-item {
    padding: 0 10px;
}

.photos-slider .photo-card {
    height: 100%;
    transition: transform 0.3s ease;
    border-radius: 12px;
    overflow: hidden;
...
.photos-slider .photo-card:hover {
    transform: translateY(-5px);
}

.photos-slider .photo-image {
    width: 100%;
    height: 250px;
    object-fit: cover;
    transition: transform 0.3s ease;
...
.photos-slider .photo-card:hover .photo-image {
...
window.addEventListener('load', function() {
    console.log('🔄 Homepage loaded, initializing sliders...');
    ...
    if ($slider.length > 0) {
        ...
        console.log('Events slider initialized successfully');
    } else {
        console.error('Events slider not found!');
    }

    // Inizializza lo slider delle foto
    const $photosSlider = $('.photos-slider');
    console.log('Photos slider found:', $photosSlider.length);
    ...
    if ($photosSlider.length > 0) {
        ...
        console.log('Photos slider initialized successfully');
    } else {
        console.error('Photos slider not found!');
    }
...
<?php endif; ?>
        ...
        <!-- Recent Photos Slider Section -->
        <?php if($recentPhotos->count() > 0): ?>
        <!-- Debug: <?php echo e($recentPhotos->count()); ?> foto trovate -->
        <div class="row mb-4">
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <a href="<?php echo e(route('media.index')); ?>" class="btn btn-sm btn-primary hover-effect">
                    ...
                    <div class="card-body">
                        <div class="photos-slider app-arrow" id="photos-slider">
                            ...
                            <div class="autoplay-item">
                                <div class="photo-card hover-effect shadow-sm">
                                    <div class="position-relative">
                                        <img src="<?php echo e($photo->image_url); ?>" class="photo-image" alt="<?php echo e($photo->alt_text ?: $photo->title ?: 'Foto di ' . $photo->user->name); ?>">
                                        <div class="position-absolute top-0 start-0 end-0 bottom-0 bg-dark opacity-0 transition-all" style="transition: opacity 0.3s ease;"></div>
                                        <div class="position-absolute bottom-0 start-0 end-0 p-3 text-white" style="background: linear-gradient(transparent, rgba(0,0,0,0.7));">
                                            <div class="d-flex align-items-center">
                                                <?php if($photo->user->profile_photo): ?>
                                                    <div class="h-24 w-24 d-flex-center b-r-50 overflow-hidden me-2">
                                                        <img src="<?php echo e($photo->user->profile_photo_url); ?>" alt="<?php echo e($photo->user->name); ?>" class="w-100 h-100" style="object-fit: cover;">
                                                    </div>
                                                <?php else: ?>
                                                    <div class="h-24 w-24 d-flex-center b-r-50 overflow-hidden bg-gradient-primary me-2">
                                                        <span class="text-white fw-bold f-s-12"><?php echo e(substr($photo->user->name, 0, 2)); ?></span>
                                                    </div>
                                                <?php endif; ?>
                                                <div class="flex-grow-1">
                                                    <h6 class="mb-0 f-w-600 f-s-12 text-white"><?php echo e($photo->user->name); ?></h6>
                                                    <small class="text-white-50 f-s-10"><?php echo e($photo->created_at->diffForHumans()); ?></small>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-12 col-lg-6 mb-3 mb-lg-0">
                                        <div class="position-relative">
                                            <div class="position-relative overflow-hidden rounded-3" style="aspect-ratio: 16/9;">
                                                <?php if($mostPopularVideo->thumbnail_path || $mostPopularVideo->peertube_thumbnail_url): ?>
