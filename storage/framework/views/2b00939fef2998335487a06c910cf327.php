...
<div class="social-comment-btn"
     ...
     data-unique-id="<?php echo e($uniqueId); ?>"
     onclick="showCommentsModal_<?php echo e($uniqueId); ?>(event)"
...
function showCommentsModal_<?php echo e($uniqueId); ?>(event) {
    event.stopPropagation(); // Previene l'apertura del modal
    openCommentsModal_<?php echo e($uniqueId); ?>('<?php echo e($contentType); ?>', <?php echo e($content->id); ?>);
}

// Apre il modal dei commenti
async function openCommentsModal_<?php echo e($uniqueId); ?>(mediaType, mediaId) {
    const modalId = 'commentsModal_<?php echo e($uniqueId); ?>';

    // Crea il modal se non esiste
    if (!document.getElementById(modalId)) {
        createCommentsModal_<?php echo e($uniqueId); ?>();
    }

    // Imposta i valori nel form
    document.getElementById('commentMediaType_<?php echo e($uniqueId); ?>').value = mediaType;
    document.getElementById('commentMediaId_<?php echo e($uniqueId); ?>').value = mediaId;

    // Aggiorna il titolo del modal
    const modalTitle = document.getElementById('commentsModalLabel_<?php echo e($uniqueId); ?>');
    const typeNames = {
        'video': 'Video',
        'photo': 'Foto',
        'article': 'Articolo',
    ...
    // Mostra loading
    document.getElementById('commentsLoading_<?php echo e($uniqueId); ?>').style.display = 'block';
    document.getElementById('commentsError_<?php echo e($uniqueId); ?>').style.display = 'none';
    document.getElementById('commentsContainer_<?php echo e($uniqueId); ?>').style.display = 'none';
    ...
    // Carica i commenti
    await loadComments_<?php echo e($uniqueId); ?>(mediaType, mediaId);
}

// Carica i commenti usando il sistema unificato
async function loadComments_<?php echo e($uniqueId); ?>(mediaType, mediaId) {
    try {
        ...
        if (response.ok && data.success) {
            displayComments_<?php echo e($uniqueId); ?>(data.comments);
        } else {
            throw new Error(data.message || 'Errore nel caricamento dei commenti');
        }
    } catch (error) {
        console.error('Errore caricamento commenti:', error);
        showCommentsError_<?php echo e($uniqueId); ?>(error.message);
    } finally {
        document.getElementById('commentsLoading_<?php echo e($uniqueId); ?>').style.display = 'none';
    }
}

// Visualizza i commenti
function displayComments_<?php echo e($uniqueId); ?>(comments) {
    const commentsList = document.getElementById('commentsList_<?php echo e($uniqueId); ?>');
    ...
    document.getElementById('commentsContainer_<?php echo e($uniqueId); ?>').style.display = 'block';
}

// Mostra errore nei commenti
function showCommentsError_<?php echo e($uniqueId); ?>(message) {
    document.getElementById('commentsError_<?php echo e($uniqueId); ?>').innerHTML = `
        <div class="alert alert-danger">
            <i class="ph-duotone ph-warning-circle me-2"></i>
            ${message}
        </div>
    `;
    document.getElementById('commentsError_<?php echo e($uniqueId); ?>').style.display = 'block';
}

// Crea il modal dei commenti se non esiste
function createCommentsModal_<?php echo e($uniqueId); ?>() {
    const modalHtml = `
        <div class="modal fade" id="commentsModal_<?php echo e($uniqueId); ?>" tabindex="-1" aria-labelledby="commentsModalLabel_<?php echo e($uniqueId); ?>" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="commentsModalLabel_<?php echo e($uniqueId); ?>">
                    ...
                    <div class="modal-body">
                        <!-- Loading -->
                        <div id="commentsLoading_<?php echo e($uniqueId); ?>" class="text-center py-4">
                        ...
                        <!-- Error -->
                        <div id="commentsError_<?php echo e($uniqueId); ?>" style="display: none;"></div>

                        <!-- Comments Container -->
                        <div id="commentsContainer_<?php echo e($uniqueId); ?>" style="display: none;">
                            <!-- Lista commenti -->
                            <div id="commentsList_<?php echo e($uniqueId); ?>" class="mb-3">
                            ...
                            <div class="border-top pt-3">
                                ...
                                <form id="newCommentForm_<?php echo e($uniqueId); ?>">
                                    <input type="hidden" id="commentMediaType_<?php echo e($uniqueId); ?>" value="">
                                    <input type="hidden" id="commentMediaId_<?php echo e($uniqueId); ?>" value="">
                                    <div class="mb-3">
                                        <textarea class="form-control" id="commentContent_<?php echo e($uniqueId); ?>" rows="3" placeholder="Scrivi il tuo commento..." maxlength="500"></textarea>
                                        <div class="form-text">
                                            <span id="commentCharCount_<?php echo e($uniqueId); ?>">0</span>/500 caratteri
                                        </div>
                                    </div>
                                    <div class="d-flex justify-content-between align-items-center">
                                        <button type="submit" class="btn btn-primary" id="submitCommentBtn_<?php echo e($uniqueId); ?>">
    ...
    setupCommentForm_<?php echo e($uniqueId); ?>();
}

// Configura il form per i commenti
function setupCommentForm_<?php echo e($uniqueId); ?>() {
    const form = document.getElementById('newCommentForm_<?php echo e($uniqueId); ?>');
    const textarea = document.getElementById('commentContent_<?php echo e($uniqueId); ?>');
    const charCount = document.getElementById('commentCharCount_<?php echo e($uniqueId); ?>');
    ...
    form.addEventListener('submit', async function(e) {
        e.preventDefault();
        await submitComment_<?php echo e($uniqueId); ?>();
    });
}

// Invia nuovo commento
async function submitComment_<?php echo e($uniqueId); ?>() {
    ...
    const mediaType = document.getElementById('commentMediaType_<?php echo e($uniqueId); ?>').value;
    const mediaId = document.getElementById('commentMediaId_<?php echo e($uniqueId); ?>').value;
    const content = document.getElementById('commentContent_<?php echo e($uniqueId); ?>').value.trim();
    const submitBtn = document.getElementById('submitCommentBtn_<?php echo e($uniqueId); ?>');
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Reset form
            document.getElementById('commentContent_<?php echo e($uniqueId); ?>').value = '';
            document.getElementById('commentCharCount_<?php echo e($uniqueId); ?>').textContent = '0';

            // Aggiorna il contatore nel pulsante commenti
            const commentButton = document.querySelector(`.social-comment-btn[data-unique-id="<?php echo e($uniqueId); ?>"] .comment-count`);
...
<?php /**PATH C:\xampp\htdocs\slamin\resources\views/components/social-comment-button.blade.php ENDPATH**/ ?>