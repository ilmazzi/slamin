...
<!-- WebSocket Client -->
<script src="<?php echo e(asset('assets/js/websocket-client.js')); ?>"></script>
                        ...
                        <div class="card-header">
                            <div class="d-flex align-items-center">
                                <span class="chatdp h-30 w-30 d-flex-center b-r-50 position-relative bg-danger">
                                    ...
                                    <span class="position-absolute top-0 end-0 p-1 bg-<?php echo e(auth()->user()->getOnlineStatusColor()); ?> border border-light rounded-circle online-status-indicator" 
                                          data-user-id="<?php echo e(auth()->user()->id); ?>" 
                                          title="<?php echo e(auth()->user()->getLastSeenDisplay()); ?>">
                                        <i class="ph <?php echo e(auth()->user()->getOnlineStatusIcon()); ?> f-s-8"></i>
                                    </span>
                                </span>
                                ...
                                <div>
                                    ...
                                    <div class="btn-group dropdown-icon-none">
                                        <button type="button" class="btn btn-light btn-sm dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                                            <i class="ph <?php echo e(auth()->user()->getOnlineStatusIcon()); ?> fs-5 text-<?php echo e(auth()->user()->getOnlineStatusColor()); ?>"></i>
                                        </button>
                        ...
                        <div class="card-body">
                            ...
                            <div class="content-wrapper">
                                ...
                                <div id="tab-1" class="tabs-content active">
                                                ...
                                                <div class="tab-pane fade show active" id="private-tab-pane" role="tabpanel" aria-labelledby="private-tab" tabindex="0">
                                                    <div class="chat-contact" id="privateChatsList">
                                                        <?php $__empty_1 = true; $__currentLoopData = $chats->where('type', 'private'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $chat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                                            ...
                                                            ?>
                                                            <div class="chat-contactbox" data-chat-id="<?php echo e($chat->id); ?>" data-user-id="<?php echo e($otherUser ? $otherUser->id : ''); ?>" onclick="loadChat(<?php echo e($chat->id); ?>)">
                                    ...
                                    <?php if($otherUser): ?>
                                        <span class="position-absolute top-0 end-0 p-1 bg-<?php echo e($otherUser->getOnlineStatusColor()); ?> border border-light rounded-circle online-status-indicator" 
                                              data-user-id="<?php echo e($otherUser->id); ?>" 
                                              data-status="<?php echo e($otherUser->getOnlineStatusDisplay()); ?>"
                                              title="<?php echo e($otherUser->getLastSeenDisplay()); ?>">
                                            <i class="ph <?php echo e($otherUser->getOnlineStatusIcon()); ?> f-s-8"></i>
                                        </span>
                                    <?php else: ?>
...
function updateChatHeader(chatId) {
    ...
    if (userId) {
        fetch(`<?php echo e(route('online-status.user-status', ':userId')); ?>`.replace(':userId', userId), {
            method: 'GET',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                const statusText = data.is_online ? '<?php echo e(__("chat.online")); ?>' : '<?php echo e(__("chat.offline")); ?>';
                ...
                $('#chatHeaderStatus').text('<?php echo e(__("chat.offline")); ?>').removeClass('text-success text-muted').addClass('text-muted');
...
function refreshChatList() {
    $.get('<?php echo e(route("chat.list")); ?>')
        .done(function(response) {
            if (response.success) {
...
function updatePrivateChatsList(chats) {
    ...
    if (chats.length === 0) {
        container.html('<div class="text-center py-3"><p class="text-muted"><?php echo e(__("chat.no_private_chats")); ?></p></div>');
        return;
    }
...
function updateGroupChatsList(chats) {
    ...
    if (chats.length === 0) {
        container.html('<div class="text-center py-3"><p class="text-muted"><?php echo e(__("chat.no_group_chats")); ?></p></div>');
        return;
    }
...
function createPrivateChatHtml(chat) {
    ...
    return `
        <div class="chat-contactbox" data-chat-id="${chat.id}" data-user-id="${otherUser ? otherUser.id : ''}" onclick="loadChat(${chat.id})">
            <div class="position-absolute">
                <span class="h-30 w-30 d-flex-center b-r-50 position-relative bg-primary">
                    <img src="${otherUser ? otherUser.avatar_url : '<?php echo e(asset("assets/images/avatar/14.png")); ?>'}" alt="" class="img-fluid b-r-50">
                    ${otherUser ? `
            ...
            <div class="flex-grow-1 text-start mg-s-50">
                <p class="mb-0 f-w-500 text-dark txt-ellipsis-1">${otherUser ? otherUser.name : '<?php echo e(__("chat.unknown_user")); ?>'}</p>
                <p class="text-secondary mb-0 f-s-12 mb-0 chat-message">
                    <i class="ti ti-checks"></i>
                    ${lastMessage ? lastMessage.message.substring(0, 30) + (lastMessage.message.length > 30 ? '...' : '') : '<?php echo e(__("chat.no_messages")); ?>'}
                    ${unreadCount > 0 ? `<span class="badge bg-danger ms-1">${unreadCount}</span>` : ''}
                </p>
            </div>
            <div>
                <p class="f-s-12 chat-time">
                    ${lastMessage ? lastMessage.created_at : '<?php echo e(__("chat.no_messages")); ?>'}
                </p>
            </div>
        </div>
    `;
...
function updateUserOnlineStatus(status) {
    fetch('<?php echo e(route("online-status.update-status")); ?>', {
        method: 'POST',
        headers: {
...
function updateLastSeen() {
    fetch('<?php echo e(route("online-status.update-last-seen")); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
...
function updateOnlineStatuses() {
    ...
    fetch('<?php echo e(route("online-status.multiple-users-status")); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    ...
    Object.keys(statuses).forEach(userId => {
        ...
        if (currentChatUserId && currentChatUserId == userId) {
            ...
            switch(status.status_display) {
                case 'Online':
                    statusText = '<?php echo e(__("chat.online")); ?>';
                ...
                case 'Invisibile':
                case 'Offline':
                    statusText = '<?php echo e(__("chat.offline")); ?>';
                    statusClass = 'text-muted';
                    indicatorClass = 'bg-secondary';
                    break;
                default:
                    statusText = status.is_online ? '<?php echo e(__("chat.online")); ?>' : '<?php echo e(__("chat.offline")); ?>';
...
function changeOnlineStatus(status) {
    ...
    const ownIndicator = $('.online-status-indicator[data-user-id="<?php echo e(auth()->id()); ?>"]');
    ...
    if (currentChatUserId && currentChatUserId == currentUserId) {
        ...
        const statusText = status === 'invisible' ? '<?php echo e(__("chat.offline")); ?>' : '<?php echo e(__("chat.online")); ?>';
        ...
        if (data.success) {
            ...
            if (unreadCount > 0) {
                ...
                if (badge.length > 0) {
                    ...
                    $('.sidebar a[href="<?php echo e(route("chat.index")); ?>"]').append(
...
function loadPrivacySettings() {
    fetch('<?php echo e(route("online-status.get-privacy-preferences")); ?>', {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
...
function savePrivacySettings() {
    ...
    fetch('<?php echo e(route("online-status.privacy-preferences")); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
