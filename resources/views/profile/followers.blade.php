        ...
        <div class="row m-1">
            <div class="col-12">
                ...
                <ul class="app-line-breadcrumbs mb-3">
                    ...
                    <li class="active">
                        <a href="#" class="f-s-14 f-w-500">{{ __('profile.followers') }}</a>
                    </li>
                </ul>
            </div>
        </div>
                ...
                <div class="card hover-effect h-100">
                    <div class="card-body text-center">
                        ...
                        <h5 class="card-title f-w-600 mb-2">
                            <a href="{{ route('user.show', $follower) }}" class="text-decoration-none">
                                {{ $follower->getDisplayName() }}
                            </a>
                        </h5>

                        @if($follower->nickname && $follower->nickname !== $follower->name)
                        <p class="text-muted f-s-14 mb-2">{{ $follower->nickname }}</p>
                        @endif
                        ...
                        <div class="row text-center mb-3">
                            <div class="col-4">
                                ...
                                <div class="fw-bold">{{ $follower->videos_count }}</div>
                            </div>
                            <div class="col-4">
                                <div class="f-s-12 text-muted">Foto</div>
                                <div class="fw-bold">{{ $follower->photos_count }}</div>
                            </div>
                            <div class="col-4">
                                <div class="f-s-12 text-muted">Poesie</div>
                                <div class="fw-bold">{{ $follower->poems_count }}</div>
                            </div>
                        </div>

                        <!-- Azioni -->
                        <div class="d-flex justify-content-center gap-2">
                            <a href="{{ route('user.show', $follower) }}" class="btn btn-primary btn-sm">
                                <i class="ph-duotone ph-user me-1"></i>
                                Profilo
                            </a>
                            @auth
                            <button type="button"
                                    class="btn {{ $follower->is_followed_by_current_user ?? false ? 'btn-success' : 'btn-outline-primary' }} btn-sm"
                                    onclick="followUser({{ $follower->id }})"
                                    id="followBtn{{ $follower->id }}">
                                <i class="ti {{ $follower->is_followed_by_current_user ?? false ? 'ti-user-check' : 'ti-user' }} me-1"></i>
                                <span id="followText{{ $follower->id }}">
                                    {{ $follower->is_followed_by_current_user ?? false ? 'Following' : 'Follow' }}
                                </span>
                            </button>
                            @else
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body text-center py-5">
                        ...
                        <p class="text-muted">Questo utente non ha ancora followers.</p>
                        @auth
                        @if(!$isOwnProfile)
