                ...
                <div class="card-body p-0">
                    ...
                    <img src="{{ \App\Helpers\AvatarHelper::getUserAvatarUrl($user) }}"
                        ...
                        <div class="person-details">
                            ...
                            {{-- Level & Gamification Stats Mobile --}}
                            @php
                                $userPointsMobile = $user->userPoints;
                                $profileBadgesMobile = $user->badges()
                                    ->wherePivot('show_in_profile', true)
                                    ->orderBy('user_badges.profile_order')
                                    ->orderBy('badges.order')
                                    ->orderByDesc('user_badges.earned_at')
                                    ->get();
                            @endphp
                            
                            @if($userPointsMobile || $profileBadgesMobile->count() > 0)
                            <div class="mt-3 pt-3 border-top">
                                {{-- User Level Mobile --}}
                                @if($userPointsMobile)
                                <div class="mb-3">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <h6 class="mb-2 f-s-14">
                                            <i class="ph-duotone ph-chart-line me-1 text-info"></i>
                                            {{ __('gamification.level') }}
                                        </h6>
                                        <span class="badge {{ $userPointsMobile->level >= 5 ? 'bg-gradient-warning' : 'bg-light-info' }}">
                                            Livello {{ $userPointsMobile->level }}
                                        </span>
                                    </div>
                                    @if($userPointsMobile->current_level)
                                    <p class="text-muted small mb-2">{{ $userPointsMobile->current_level->name }}</p>
                                    @endif
                                    <div class="d-flex gap-3 small">
                                        <span>
                                            <i class="ph ph-star-four text-warning me-1"></i>
                                            <strong>{{ number_format($userPointsMobile->total_points) }}</strong> punti
                                        </span>
                                        <span>
                                            <i class="ph ph-trophy text-info me-1"></i>
                                            <strong>{{ $userPointsMobile->badges_count }}</strong> badge
                                        </span>
                                    </div>
                                </div>
                                @endif
                                ...
                                @if($profileBadgesMobile->count() > 0)
                                <div>
                    ...
                    <div class="tab-wrapper">
                        <ul class="profile-app-tabs">
                            ...
                            <li class="tab-link fw-medium f-s-16 f-w-600" data-tab="2">
                                <i class="ph ph-book-open fw-bold"></i> {{ __('profile.poems') }}
                            ...
                            <li class="tab-link fw-medium f-s-16 f-w-600" data-tab="4">
                                ...
                                @if(($user->photos()->approved()->count() + $videos->count()) > 0)
                                <span class="badge rounded-pill bg-info badge-notification">
                                    {{ $user->photos()->approved()->count() + $videos->count() }}
                                </span>
                                @endif
                            </li>
            ...
            <div class="content-wrapper">
                    ...
                    <img src="{{ \App\Helpers\AvatarHelper::getUserAvatarUrl($user) }}"
                         ...
                         style="object-fit: cover;">
                                    ...
                                    <div class="person-details">
                                        ...
                                        @if($userPoints || $profileBadges->count() > 0)
                                        <div class="mt-3 pt-3 border-top">
                                            {{-- User Level --}}
                                            @if($userPoints)
                                            <div class="mb-3">
                                                <div class="d-flex align-items-center justify-content-between">
                                                    ...
                                                    <span class="badge {{ $userPoints->level >= 5 ? 'bg-gradient-warning' : 'bg-light-info' }}">
                                                        Livello {{ $userPoints->level }}
                                                    </span>
                                                </div>
                                                @if($userPoints->current_level)
                                                <p class="text-muted small mb-2">{{ $userPoints->current_level->name }}</p>
                                                @endif
                                                <div class="d-flex gap-3 small">
                                                    <span>
                                                        <i class="ph ph-star-four text-warning me-1"></i>
                                                        <strong>{{ number_format($userPoints->total_points) }}</strong> punti
                                                    </span>
                                                    <span>
                                                        <i class="ph ph-trophy text-info me-1"></i>
                                                        <strong>{{ $userPoints->badges_count }}</strong> badge
                                                    </span>
                                                </div>
                                            </div>
                                            @endif
                ...
                <div id="tab-2" class="tabs-content">
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex justify-content-between align-items-center">
                                <h5 class="mb-0">{{ __('profile.poems') }}</h5>
                                @if($isOwnProfile)
                                <a href="{{ route('poems.create') }}" class="btn btn-sm btn-primary hover-effect">
                                    <i class="ti ti-plus"></i> {{ __('profile.add_poem') }}
                                </a>
                                @endif
                            </div>
                        </div>
                        <div class="card-body">
                            ...
                            @if($stats['total_poems'] > 6)
                            <div class="text-center mt-3">
                                <a href="{{ route('poems.index', ['user' => $user->id]) }}" class="btn btn-primary">
                                    {{ __('profile.view_all_poems') }}
                                </a>
                            </div>
                            @endif
                            @else
                            <div class="text-center py-5">
                                ...
                                <h5 class="mt-3 text-muted">{{ __('profile.no_poems') }}</h5>
                                @if($isOwnProfile)
                                <p class="text-muted">{{ __('profile.no_poems_description') }}</p>
                                <a href="{{ route('poems.create') }}" class="btn btn-primary">
                                    <i class="ti ti-plus"></i> {{ __('profile.create_first_poem') }}
                                </a>
                                @endif
                            </div>
                            @endif
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex justify-content-between align-items-center">
                                <h5 class="mb-0">{{ __('profile.organized_events_title') }}</h5>
                                @if($isOwnProfile)
                                <a href="{{ route('events.create') }}" class="btn btn-sm btn-primary hover-effect">
                                    <i class="ti ti-plus"></i> {{ __('profile.create_event') }}
                                </a>
                                @endif
                            </div>
                        </div>
                        <div class="card-body">
                            @if($stats['total_events'] > 0)
                            <div class="row">
                                @foreach($user->events()->latest()->take(6)->get() as $event)
                                <div class="col-md-6 col-lg-4 mb-3">
                            ...
                            <div class="text-center mt-3">
                                <a href="{{ route('events.index', ['organizer' => $user->id]) }}" class="btn btn-primary">
                                    {{ __('profile.view_all_events') }}
                                </a>
                            </div>
                            @endif
                            ...
                            <div class="text-center py-5">
                                ...
                                <h5 class="mt-3 text-muted">{{ __('profile.no_events') }}</h5>
                                @if($isOwnProfile)
                                <p class="text-muted">{{ __('profile.no_events_description') }}</p>
                                <a href="{{ route('events.create') }}" class="btn btn-primary">
                                    <i class="ti ti-plus"></i> {{ __('profile.create_first_event') }}
                                </a>
                                @endif
                            </div>
                            @endif
