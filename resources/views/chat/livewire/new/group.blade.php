            ...
            <form wire:submit="validateDetails()" class="flex flex-col  h-full p-4">
                    ...
                    <div class="flex gap-10 w-full">
                        ...
                        @if ($photo)
                            <div class="relative  w-28 h-28 overflow-clip rounded-full">
                                <x-chat::avatar :src="$photo->temporaryUrl()" class="w-28 h-28" />
                                <button
                                    type="button"
                        ...
                        @else
                            <label class="cursor-pointer">
                                <x-chat::avatar wire:loading.class="animate-pulse" wire:target="photo" :group="true" class="w-28 h-28" />
                        ...
                        <div class=" my-auto">
                            ...
                            <label for="name">@lang('chat::new.group.inputs.name.label')</label>

                            <input id='name' type="text" wire:model='name' autofocus placeholder="{{__('chat::new.group.inputs.name.placeholder') }}"
                ...
                <main class="my-5">
                    <div class=" my-auto flex flex-col gap-y-2">
                        ...
                        <label class="my-2" for="description">@lang('chat::new.group.inputs.description.label')</label>

                        <textarea id='description' type="text" wire:model='description' placeholder="{{__('chat::new.group.inputs.description.placeholder')}}" rows="4"
                ...
                <footer class="flex gap-4 justify-end mt-auto">
                    <x-chat::actions.close-modal>
                        <button type="button" dusk="cancel_create_new_group_button"class="font-bold cursor-pointer hover:bg-[var(--wc-light-secondary)] dark:hover:bg-[var(--wc-dark-secondary)] p-3 px-4 rounded-xl ">
                            @lang('chat::new.group.actions.cancel.label')
                        </button>
                    </x-chat::actions.close-modal>
                    ...
                    <button type="submit" :disabled="!($wire.name?.trim()?.length)" dusk="next_button"
                        ...
                        @lang('chat::new.group.actions.next.label')
                    </button>
                </footer>
            </form>
        ...
        <section dusk="add_members_section" x-cloak x-show="$wire.showAddMembers==true"
            ...
            <header class=" sticky top-0 bg-[var(--wc-light-primary)] dark:bg-[var(--wc-dark-primary)] z-10 py-2">
                <div class="flex items-center pb-2">
                    ...
                    <h3 class="text-sm mx-auto font-semibold "><span>@lang('chat::new.group.labels.add_members')</span> {{count($selectedMembers)}} / {{$maxGroupMembers}}</h3>
                    ...
                    <button 
                        ...
                        @lang('chat::new.group.actions.create.label')
                ...
                <div
                 ...
                 class="text-red-500 text-sm mx-auto ">
                   <span x-transition x-show="showError">
                    @lang('chat::new.group.messages.members_limit_error',['count'=>$maxGroupMembers])
                   </span>
                </div>
                {{-- Search input --}}
                <section class="flex flex-wrap items-center px-0 border-b border-[var(--wc-light-secondary)] dark:border-[var(--wc-dark-secondary)]">
                    <input type="search" id="users-search-field" wire:model.live.debounce='search' autocomplete="off"
                        placeholder="{{__('chat::new.group.inputs.search.placeholder')}}"
            ...
            <div class="relative w-full">
                ...
                <section class="my-4 grid">
                    ...
                    @else
                        @if (!empty($search))
                        <span class="m-auto">@lang('chat::new.group.messages.empty_search_result')</span>
                        @endif
                    @endif
                </section>
            </div>
