...
@section('title', 'Dettagli Log - Admin')
                            ...
                            <div class="col-md-6">
                                <table class="table table-sm">
                                    ...
                                    <tr>
                                        ...
                                        <td><span class="badge bg-light-primary">{{ $log->category }}</span></td>
                                    </tr>
                                    <tr>
                            ...
                            <div class="col-md-6">
                                <table class="table table-sm">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($log->url)
                                                <a href="{{ $log->url }}" target="_blank" class="text-truncate d-inline-block" style="max-width: 200px;">
                                                    {{ $log->url }}
                                                </a>
                                            @else
                                    ...
                                    <tr>
                                        ...
                                        <td><span class="badge bg-light-secondary">{{ $log->method }}</span></td>
                                    </tr>
                                    <tr>
                ...
                <div class="card hover-effect mb-4">
                    ...
                    <div class="card-body">
                        <div class="d-grid gap-2">
                            <a href="{{ route('admin.logs.activity') }}" class="btn btn-primary">
                                <i class="ph ph-arrow-left me-1"></i>Torna ai Log
                            </a>
                            @if($log->related_model && $log->related_id)
                                <a href="#" class="btn btn-info" onclick="viewRelatedModel('{{ $log->related_model }}', {{ $log->related_id }})">
                ...
                <div class="card hover-effect">
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <span class="badge bg-primary">{{ \App\Models\ActivityLog::where('action', $log->action)->count() }}</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <span>Log utente (24h):</span>
                            <span class="badge bg-info">{{ \App\Models\ActivityLog::where('user_id', $log->user_id)->where('created_at', '>=', now()->subDay())->count() }}</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center">
                            <span>Errori oggi:</span>
                            <span class="badge bg-danger">{{ \App\Models\ActivityLog::where('level', 'error')->where('created_at', '>=', now()->startOfDay())->count() }}</span>
                        </div>
                    </div>
                </div>
            </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if(is_array($value) || is_object($value))
                                            <pre class="mb-0" style="max-height: 200px; overflow-y: auto;">{{ json_encode($value, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                        ...
                        <table class="table table-sm">
                            ...
                            <tbody>
                                @foreach($similarLogs as $similarLog)
                                    <tr>
                                        <td>
                                            <small>{{ $similarLog->created_at->format('d/m/Y H:i:s') }}</small>
                                        </td>
                                        <td>
                                            @if($similarLog->user)
                                                <span class="text-primary">{{ $similarLog->user->name }}</span>
                                            @else
                                                <span class="text-muted">Guest</span>
                                            @endif
                                        </td>
                                        <td>
                                            @switch($similarLog->level)
                                                @case('critical')
                                                    <span class="badge bg-danger">Critical</span>
                                                    @break
                                                @case('error')
                                                ...
                                                @case('info')
                                                    ...
                                                    @break
                                                @default
                                                    <span class="badge bg-secondary">{{ $similarLog->level }}</span>
                                            @endswitch
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <a href="{{ route('admin.logs.show', $similarLog->id) }}" class="btn btn-sm btn-light">
                                                <i class="ph ph-eye"></i>
                                            </a>
                                        </td>
                                    </tr>
