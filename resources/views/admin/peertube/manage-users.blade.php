...
@section('title', 'Gestione Utenti PeerTube - Admin')
    ...
    <div class="row">
        <div class="col-12">
            <ul class="app-line-breadcrumbs mb-3">
                ...
                <li class="">
                    <a href="{{ route('admin.peertube.index') }}" class="f-s-14 f-w-500">
                        <span>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1 ms-3">
                            ...
                            <h4 class="mb-0">{{ $peertubeUsers->count() }}</h4>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1 ms-3">
                            ...
                            <h4 class="mb-0">{{ $allUsers->count() - $peertubeUsers->count() }}</h4>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1 ms-3">
                            ...
                            <h4 class="mb-0">{{ $allUsers->count() > 0 ? round(($peertubeUsers->count() / $allUsers->count()) * 100, 1) : 0 }}%</h4>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <select class="form-select" id="userSelect" onchange="loadUserData()">
                                ...
                                @foreach($allUsers as $user)
                                    <option value="{{ $user->id }}"
                                            data-has-peertube="{{ $user->peertube_user_id ? 'true' : 'false' }}"
...
function loadUserData() {
    ...
    fetch('{{ route("admin.peertube.show-user") }}', {
        method: 'POST',
        headers: {
...
function updateUserData() {
    ...
    fetch('{{ route("admin.peertube.update-user-data") }}', {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function createUserAccount(forceRecreate = false) {
    ...
    fetch('{{ route("admin.peertube.create-user-account") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function deletePeerTubeUser() {
    ...
    fetch('{{ route("admin.peertube.delete-user") }}', {
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function changeUserEmail(newEmail) {
    ...
    fetch('{{ route("admin.peertube.change-user-email") }}', {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function verifyUserExists() {
    ...
    fetch('{{ route("admin.peertube.verify-user-exists") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function syncUserData() {
    ...
    fetch('{{ route("admin.peertube.sync-user-data") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
