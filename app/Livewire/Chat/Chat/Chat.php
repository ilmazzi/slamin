...
class Chat extends Component
{
    ...
    {
        return [
            'body.required' => __('chat::validation.required', ['attribute' => __('chat::chat.inputs.message.label')]),
            'media.max' => __('chat::validation.max.array', ['attribute' => __('chat::chat.inputs.media.label')]),
            'media.*.max' => __('chat::validation.max.file', ['attribute' => __('chat::chat.inputs.media.label')]),
            'media.*.mimes' => __('chat::validation.mimes', ['attribute' => __('chat::chat.inputs.media.label')]),
            'files.max' => __('chat::validation.max.array', ['attribute' => __('chat::chat.inputs.files.label')]),
            'files.*.max' => __('chat::validation.max.file', ['attribute' => __('chat::chat.inputs.files.label')]),
            'files.*.mimes' => __('chat::validation.mimes', ['attribute' => __('chat::chat.inputs.files.label')]),
    ...
    public function exitConversation()
    {
        ...
        abort_unless($this->conversation->isGroup(), 403, __('chat::chat.messages.cannot_exit_self_or_private_conversation'));

        // make sure owner if group cannot be removed from chat
        abort_if($auth->isOwnerOf($this->conversation), 403, __('chat::chat.messages.owner_cannot_exit_conversation'));
        ...
        if ($this->isWidget()) {
            ...
            $this->redirectRoute(ChatHelper::indexRouteName());
        }
    }
    ...
    protected function rateLimit()
    {
        ...
        if (RateLimiter::tooManyAttempts('send-message:'.auth()->id(), $perMinute)) {
            ...
            return abort(429, __('chat::chat.messages.rate_limit'));
        }
        ...
        if (count($attachments) != 0) {
            ...
            foreach ($attachments as $key => $attachment) {
                ...
                $path = $attachment->store(
                    ChatHelper::storageFolder(),
                    ChatHelper::storageDisk()
                );
                ...
                $attachment = $message->attachment()->create([
                    ...
                    'url' => Storage::disk(ChatHelper::storageDisk())->url($path), // Use disk and path
                ]);
    ...
    private function messageGroupKey(Message $message): string
    {
        ...
        if ($messageDate->isToday()) {
            $groupKey = __('chat::chat.message_groups.today');
        } elseif ($messageDate->isYesterday()) {
            $groupKey = __('chat::chat.message_groups.yesterday');
    ...
    {
        return view('chat::components.placeholders.chat');
    }
    ...
    private function initializeConversation($conversation)
    {
        ...
        if ($conversation instanceof Conversation) {
            ...
            if (! $this->conversation) {
                abort(404, __('chat::chat.messages.conversation_not_found')); // Custom error response
            }
        } elseif (is_null($conversation)) {
            abort(422, __('chat::chat.messages.conversation_id_required')); // Custom error for missing input
        } else {
            return abort(422, __('chat::chat.messages.invalid_conversation_input')); // Handle invalid input types
        }
