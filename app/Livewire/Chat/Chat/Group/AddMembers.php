...
use App\Livewire\Chat\Chat\Info;
...
class AddMembers extends ModalComponent
{
    ...
    public function toggleMember($id, string $class)
    {
        ...
        if ($model) {
            ...
            if ($this->selectedMembers->contains(fn ($member) => $member->id == $model->id && get_class($member) == get_class($model))) {
                ...
                if ($this->newTotalCount >= ChatHelper::maxGroupMembers()) {
    ...
    public function render()
    {
        ...
        return view('chat::livewire.chat.group.add-members', ['maxGroupMembers' => ChatHelper::maxGroupMembers()]);
    }
}
