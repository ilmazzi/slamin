...
class Group extends ModalComponent
{
    ...
    {
        return [
            'name.required' => __('chat::validation.required', ['attribute' => __('chat::chat.group.info.inputs.name.label')]),
            'name.max' => __('chat::validation.max.string', ['attribute' => __('chat::chat.group.info.inputs.name.label')]),
    ...
    public function toggleMember($id, string $class)
    {
        ...
        if ($model) {
            if ($this->selectedMembers->contains(fn ($member) => $member->id == $model->id && get_class($member) == get_class($model))) {
                ...
                if (count($this->selectedMembers) >= ChatHelper::maxGroupMembers()) {
    ...
    public function render()
    {
        ...
        return view('chat::livewire.new.group', ['maxGroupMembers' => ChatHelper::maxGroupMembers()]);
    }
}
