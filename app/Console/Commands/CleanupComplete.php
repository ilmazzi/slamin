...
class CleanupComplete extends Command
{
    protected $signature = 'db:cleanup-complete {--force} {--dry-run}';
    protected $description = 'Pulizia completa del database mantenendo solo utenti e configurazioni';
    ...
    public function handle()
    {
        ...
        $this->line('');
        $this->line('🧹 PULIZIA COMPLETA DATABASE');
        $this->line('');
    ...
    private function cleanupAllData($dryRun)
    {
        $this->line('🗑️  Eliminazione di tutti i dati...');
        ...
        foreach ($tablesToClean as $table) {
            try {
                ...
                if (!$dryRun && $count > 0) {
                    // Prova prima con TRUNCATE, se fallisce usa DELETE
                    try {
                        DB::statement('SET FOREIGN_KEY_CHECKS=0;');
                        DB::table($table)->truncate();
                        DB::statement('SET FOREIGN_KEY_CHECKS=1;');
                        $this->line("   ✅ {$count} record eliminati da {$table} (TRUNCATE)");
                    } catch (\Exception $e) {
                        // Se TRUNCATE fallisce, usa DELETE
                        DB::table($table)->delete();
                        $this->line("   ✅ {$count} record eliminati da {$table} (DELETE)");
                    }
                } elseif ($dryRun) {
                ...
                // Prova con DELETE come fallback
                if (!$dryRun) {
                    try {
                        DB::table($table)->delete();
                        $this->line("   ✅ Record eliminati da {$table} (DELETE fallback)");
                    } catch (\Exception $e2) {
                        $this->error("   ❌ Errore anche con DELETE per {$table}: " . $e2->getMessage());
                    }
                }
            }
        }
